...
The full policy — Linear flow, coding guidelines, Rust specifics — lives in [CLAUDE.md](./CLAUDE.md). This file exists so agents that look for `AGENTS.md` by convention find the same guidance. **Both files are kept in sync; update them together.**
